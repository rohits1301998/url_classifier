...
path = 'C:/Users/Vivek/Desktop/url/UrlClassifier-master/datasets/'
...
def create_dataset(path, label):
	...
	XY = XY + column23
	# print(XY)
	sorted_label_size = sorted(label_size.items(), key=lambda kv : (kv[1],kv[0]))
	...
	for k, v in sorted_label_size:
		...
		with open(filename, 'r') as f_in:
			next(f_in)
			column23 = []
			for cols in itertools.islice(csv.reader(f_in, delimiter=","), int(label_count)):
